...
// ═══════════════════════════════════════════════════════════════
// 🔧 시즌 설정 — 시즌 전환 시 이 섹션만 수정하면 됩니다
// ═══════════════════════════════════════════════════════════════
// 
// 시즌 옵션:
//   "winter"     → 겨울방학 윈터스쿨 (12월 초~)  : Δ0 Regular 8주
//   "spring"     → 내신 대비 중간고사 (2월 초~)   : Δ1+ΔFINAL 풀패키지
//   "spring-final" → FINAL 추가 (3월 중~)         : 풀패키지 + FINAL 단과
//   "summer"     → 여름방학 썸머스쿨 (7월 초~)    : Δ0 Fast 5주
//   "fall"       → 내신 대비 기말고사 (8월 초~)   : Δ1+ΔFINAL 풀패키지
//   "fall-final" → FINAL 추가 (9월 중~)           : 풀패키지 + FINAL 단과
//
const CURRENT_SEASON = "spring-final";
// ═══════════════════════════════════════════════════════════════

// ─── 활성 과목 (5과목) ───
// 미적분2, Statistics(확률과 통계)는 이번 윈터스쿨부터 추가 예정
const subjects = [
  ...
  { id: "geometry", name: "기하" },
  // { id: "calculus-2", name: "미적분 2" },         // 윈터스쿨부터 추가
  // { id: "statistics", name: "Statistics" },        // 윈터스쿨부터 추가
];
...
const allProducts = {
  "delta-0-regular": {
    ...
    duration: "8주 과정",
    modules: ["Δ0 개념 완성"],
    features: ["실전 개념 탑재 · 기본기 완성"],
    hasGuide: false,
    accentColor: "brand-mint",
    dotColor: "bg-brand-mint",
    borderHover: "hover:border-brand-mint/30",
    ctaStyle: "bg-brand-mint text-brand-dark hover:bg-green-400 shadow-lg shadow-brand-mint/20",
  },
  "delta-0-fast": {
    key: "delta-0-fast",
    badge: "BEST",
  ...
  "delta-1-final-bundle": {
    ...
    ctaStyle: "bg-brand-blue hover:bg-blue-600 text-white shadow-lg shadow-brand-blue/20",
  },
  "delta-final-only": {
    ...
    ctaStyle: "bg-brand-surface hover:bg-brand-elevated text-white/80 border border-white/[0.08]",
  },
};

// ─── 시즌별 노출 상품 매핑 ───
const seasonConfig: Record<string, { title: string; emoji: string; productKeys: string[] }> = {
  "winter":       { title: "겨울방학 윈터스쿨", emoji: "❄️", productKeys: ["delta-0-regular"] },
  "spring":       { title: "내신 대비 · 중간고사", emoji: "📚", productKeys: ["delta-1-final-bundle"] },
  "spring-final": { title: "내신 대비 · 중간고사", emoji: "📚", productKeys: ["delta-1-final-bundle", "delta-final-only"] },
  "summer":       { title: "여름방학 썸머스쿨", emoji: "☀️", productKeys: ["delta-0-fast"] },
  "fall":         { title: "내신 대비 · 기말고사", emoji: "📚", productKeys: ["delta-1-final-bundle"] },
  "fall-final":   { title: "내신 대비 · 기말고사", emoji: "📚", productKeys: ["delta-1-final-bundle", "delta-final-only"] },
};

export default function CoursesPage() {
  const [selectedSubject, setSelectedSubject] = useState(subjects[0]);
  const season = seasonConfig[CURRENT_SEASON];
  const activeProducts = season.productKeys.map(k => allProducts[k as keyof typeof allProducts]);
  ...
  return (
    <div className="min-h-screen bg-brand-dark flex flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-16 max-w-6xl">
        ...
        {/* Active Products for Current Season */}
        <div className="mb-10">
          <div className={`grid gap-4 ${activeProducts.length === 1 ? 'grid-cols-1 max-w-xl' : 'grid-cols-1 md:grid-cols-2'}`}>
                ...
                <div className="p-6 md:p-8 flex flex-col flex-1">
                  ...
                  <div className="mt-auto pt-6 border-t border-white/[0.06]">
                    ...
                    <Link
                      ...
                      className={`flex justify-center items-center gap-2 w-full font-bold py-4 rounded-xl transition-all text-sm ${product.ctaStyle}`}
                    >
                      수강 신청하기
