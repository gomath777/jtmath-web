...
test('Given verified standard stages When selecting a web tutor route Then no provider is selected', () => {
  for (const input of [
    ...
    assert.equal(selectWebTutorRoute({ ...input, guideContext: verifiedGuideContext }).kind, 'no_model');
  }
});
