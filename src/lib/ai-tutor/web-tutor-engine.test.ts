...
test('Given a verified standard stage When running the web tutor Then it invokes the configured reasoning model', async () => {
...
test('Given a verified guide and a routed provider failure When running the web tutor Then it returns provider failure without guide content', async () => {
  const result = await runWebTutorEngine(makeInput({
  ...
  assert.equal(result.result.errorType, 'provider_error');
  assert.equal(result.result.answerText, 'synthetic provider answer');
  assert.doesNotMatch(result.result.answerText, /concept hint|start hint|decisive hint/);
});
