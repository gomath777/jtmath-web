...
export type WebTutorEngineMetadata =
  | { readonly kind: 'no_model'; readonly attemptCount: 0; readonly tokenCounts: AiTutorTokenCounts; readonly failureCategory: null }
...
export async function runWebTutorEngine(input: RunWebTutorEngineInput): Promise<WebTutorEngineAnswer> {
  ...
  if (route.kind === 'no_model') {
    return guideContext === undefined
      ? { result: providerErrorFallback(), metadata: { kind: 'provider', attemptCount: 1, tokenCounts: zeroTokenCounts, failureCategory: 'provider_error', model: null } }
      : deterministicGuideAnswer(guideContext, input.target.problemNumber);
  }

  const request = sanitizeTutorProviderRequest(createProviderRequest(scopedInput));
  const providerAnswer = await answerProvider({ input: scopedInput, request, route });
  if (isProviderFailure(providerAnswer.result) && guideContext !== undefined) {
    return { result: deterministicGuideResult(guideContext, input.target.problemNumber), metadata: providerAnswer.metadata };
  }
...
async function answerProvider(input: {
  ...
  readonly route: Exclude<ReturnType<typeof selectWebTutorRoute>, { readonly kind: 'no_model' }>;
...
function deterministicGuideAnswer(guideContext: TutorGuideContext, problemNumber: number): WebTutorEngineAnswer {
  return {
    result: deterministicGuideResult(guideContext, problemNumber),
    metadata: { kind: 'no_model', attemptCount: 0, tokenCounts: zeroTokenCounts, failureCategory: null },
  };
}

function deterministicGuideResult(guideContext: TutorGuideContext, problemNumber: number): TutorProviderResult {
  return {
    answerText: formatGuideFallbackAnswer({ guideContext, problemNumber }),
    confidence: 1,
    subjectSlug: null,
    conceptTags: [],
    errorType: null,
...
function isProviderFailure(result: TutorProviderResult): boolean {
